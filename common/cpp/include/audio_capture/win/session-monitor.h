...
template<> struct hash<flutter_webrtc_plugin::SessionKey> {
  size_t operator()(const flutter_webrtc_plugin::SessionKey &k) const
  {
...
public:
	SessionEventNotificationClient(DWORD worker_tid, SessionKey session_key)
		: worker_tid{worker_tid}, session_key{std::move(session_key)}
	{
	}
