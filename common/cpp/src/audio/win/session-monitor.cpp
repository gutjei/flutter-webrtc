...
//namespace std {
//  template <> struct hash<flutter_webrtc_plugin::SessionKey> {
//    size_t operator()(const flutter_webrtc_plugin::SessionKey& k) const noexcept {
//        size_t h1 = hash<DWORD>{}(k.pid);
//        size_t h2 = hash<std::wstring>{}(k.id);
//        // Объединяем два хеша (пример простой комбинации)
//        return h1 ^ (h2 << 1);
//    }
//};
//}
	...
	: session_control{session_control}
{
	...
	if (session_process.get() == nullptr) {
		executable = std::string("unknown");
		return;
	}

	wchar_t name_buf[MAX_PATH] = {'\0'};
	GetProcessImageFileNameW(session_process.get(), name_buf, MAX_PATH - 1);
	...
	debug("registered new session: [%d] %s", pid, executable_path.c_str());
}
