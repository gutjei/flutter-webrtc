...
#include "audio_capture/win/audio-capture-helper.h"
...
void AudioCaptureHelper::ForwardToMixers(UINT64 qpc_position, BYTE *data, UINT32 num_frames)
{
	...
	for (auto *mixer : mixers)
		mixer->SubmitPacket(qpc_position, reinterpret_cast<int16_t *>(data), num_frames);
}
...
void AudioCaptureHelper::Capture()
{
	...
	bool shutdown = false;
	while (!shutdown) {
		auto event_id = WaitForMultipleObjects(static_cast<DWORD>(events.size()), events[0].addressof(), FALSE, INFINITE);
